...
/***************************************************************************/
static unsigned char check_statement_end(void) {
	ignore_blanks();
	return (*txtpos == NL) || (*txtpos == ':');
}

/***************************************************************************/
static void printmsgNoNL(const char *msg) {
	while (*msg) {
		outchar(*msg);
		msg++;
	}
...
/***************************************************************************/
static unsigned char getln(char prompt) {
	outchar(prompt);
...
static int tinybasic_loop() {
	...
	*((unsigned short *) txtpos) = linenum;
	...
	unimplemented: printmsg(unimplimentedmsg);
	goto prompt;

	badline: printmsg(badlinemsg);
	goto prompt;

	invalidexpr: printmsg(invalidexprmsg);
	goto prompt;

	syntaxerror: printmsg(syntaxmsg);
	...
	direct: txtpos = program_end + sizeof(LINENUM);
	if (*txtpos == NL)
		goto prompt;

	interperateAtTxtpos: if (breakcheck()) {
		printmsg(breakmsg);
		goto warmstart;
	}
	...
	execnextline: if (current_line == (void *) 0) // Processing direct commands?
		goto prompt;
	current_line += current_line[sizeof(LINENUM)];

	execline: if (current_line == program_end) // Out of lines to run
		goto warmstart;
	...
	while (tempsp < memory + sizeof(memory) - 1) {
		...
		case STACK_GOSUB_FLAG:
			if (table_index == KW_RETURN) {
				struct stack_gosub_frame *f =
						(struct stack_gosub_frame *) tempsp;
		...
		case STACK_FOR_FLAG:
			...
			if (table_index == KW_NEXT) {
				...
				if (txtpos[-1] == f->for_var) {
					short int *varaddr = ((short int *) variables_table)
							+ txtpos[-1] - 'A';
...
//		printf("Poke %p value %i\n",address, (unsigned char)value);
	...
	list: linenum = testnum(); // Retuns 0 if no line found.
...
extern char interpreter_getchar(void);
extern void interpreter_putchar(char c);
...
/***********************************************************/
static void outchar(unsigned char c) {
	interpreter_putchar(c);
}

/***********************************************************/
static void tinybasic_setup() {
	printmsg("Initializing Tiny basic");
}

static void tinybasic_done() {
	printmsg("Tiny basic quit");
}

int tinybasic_interpreter(int arch, char **argv) {
	tinybasic_setup();
	while (!breakcheck())
		if (tinybasic_loop() == 0)
			break;
